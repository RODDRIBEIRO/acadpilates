...
export class DateParserFormatter extends NgbDateParserFormatter {
        ...
        ano =
          this.yyIndex === 0
            ? dateParts[0].length < 2
              ? 0
              : toInteger(dateParts[0]) + this.anoSumar
            : this.yyIndex === 1
            ? dateParts[1].length < 2
              ? 0
              : toInteger(dateParts[1]) + this.anoSumar
            : this.yyIndex === 2
            ? dateParts[2].length < 2
              ? 0
              : toInteger(dateParts[2]) + this.anoSumar
            : 0;
      }
  ...
  format(date: NgbDateStruct): string {
    ...
    if (date) {
      ...
      const stringYear = isNumber(date.year) ? (date.year - this.anoSumar).toString() : '';
