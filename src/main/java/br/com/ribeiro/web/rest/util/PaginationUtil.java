package br.com.ribeiro.web.rest.util;
...
import br.com.ribeiro.domain.AbstractAuditingEntity;
import br.com.ribeiro.security.util.JacksonUtil;
import br.com.ribeiro.service.dto.AbstractDTO;
...
public final class PaginationUtil {
	...
	public static <T> HttpHeaders generateSearchPaginationHttpHeaders(Page<T> page, String baseUrl) {
		HttpHeaders headers = new HttpHeaders();
		headers.add("X-Total-Count", Long.toString(page.getTotalElements()));
		String link = "";
		if ((page.getNumber() + 1) < page.getTotalPages()) {
	...
	public static <T> HttpHeaders generateSearchPaginationHttpHeaders(AbstractAuditingEntity query, Page<T> page,
			String baseUrl) {
		if (query != null) {
			query.setCreatedDate(null);
			query.setLastModifiedDate(null);
		}
		String escapedQuery;
		try {
			escapedQuery = URLEncoder.encode(JacksonUtil.toQueryParams(query), "UTF-8"); // TODO Converter objeto para
